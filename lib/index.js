...
function captureRetrospective(message) {
  ...
  if (message.edited || message.subtype === 'message_deleted') {
    ...
    delete retrospective.puzzle[message.ts];
          ...
          slack.users.getPresence({ token, user }, (error, presence) => {
            ...
            slack.im.open({ token, user }, (error, data) => {
              ...
              reply({ channel: dms[user] }, '👂 Tell me about your retrospective' +
                ...
                'improvement". And `?` for anything else which you can\'t category.');
            });
          });
...
function flushRetrospective(retrospective) {
  ...
  const puzzle = shuffleArray(Object.keys(retrospective.puzzle).map(_ => retrospective.puzzle[_]));

  debug('flushing retrospective (plus: %s, minus: %s, puzzles: %s)', plus.length, minus.length, puzzle.length);
  ...
  messages.push(`*Below is the "Puzzles" list, 🤔*`);
  puzzle.map((res, i) => {
    messages.push(`${i + 1}. ${res.line}`);
  });
